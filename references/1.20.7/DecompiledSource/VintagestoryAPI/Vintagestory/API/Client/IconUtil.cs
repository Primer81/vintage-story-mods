...
public class IconUtil
{
	...
	public LoadedTexture GenTexture(int width, int height, IAsset asset, double[] rgba = null)
	{
		...
		iconRendererDelegate(ctx, 0, 0, width, height, rgba ?? ColorUtil.WhiteArgbDouble);
	...
	{
		double padding = 2.0;
		double barWidth = (size - 4.0 * padding) / 5.0;
		for (int i = 0; i < 5; i++)
		{
			double xpos = x + (double)i * (padding + barWidth);
			double barHeight = (double)(i + 1) / 5.0 * size;
			ctx.NewPath();
	...
	public void Drawmenuicon_svg(Context cr, double x, double y, float width, float height, double[] rgba)
	{
		...
		float h = 144f;
		float scale = Math.Min(width / w, height / h);
		matrix.Translate(x + (double)Math.Max(0f, (width - w * scale) / 2f), y + (double)Math.Max(0f, (height - h * scale) / 2f));
		matrix.Scale(scale, scale);
		cr.Matrix = matrix;
		cr.LineWidth = 20.0;
		cr.MiterLimit = 10.0;
	...
	public void DrawPen(Context ctx, double x, double y, double lineWidth, double size)
	{
		...
		ctx.ClosePath();
		ctx.Stroke();
		double length = lineWidth / (2.0 * Math.Sqrt(2.0));
		ctx.NewPath();
	...
	{
		int blurRadius = 7;
		double innerSize = size - (double)(2 * blurRadius);
		...
		blurCtx.SetSourceRGBA(0.0, 0.0, 0.0, 0.0);
		blurCtx.Paint();
		blurCtx.LineWidth = lineWidth;
		blurCtx.SetSourceRGBA(color);
		while (quantity-- > 0)
		{
			double x2 = rnd.NextDouble() * innerSize / 2.0;
			double y2 = (double)blurRadius + rnd.NextDouble() * innerSize;
			double x3 = rnd.NextDouble() * innerSize / 2.0;
			double y3 = (double)blurRadius + rnd.NextDouble() * innerSize;
			blurCtx.NewPath();
			blurCtx.MoveTo((double)blurRadius + x2, y2);
			blurCtx.LineTo((double)blurRadius + x3, y3);
			blurCtx.ClosePath();
			blurCtx.Stroke();
			blurCtx.NewPath();
			blurCtx.MoveTo(size - (double)blurRadius - x2, y2);
			blurCtx.LineTo(size - (double)blurRadius - x3, y3);
			blurCtx.ClosePath();
			blurCtx.Stroke();
		}
