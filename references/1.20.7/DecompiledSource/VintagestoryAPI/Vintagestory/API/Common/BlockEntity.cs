...
public abstract class BlockEntity
{
	...
	public virtual void Initialize(ICoreAPI api)
	{
		...
		{
			foreach (BlockEntityBehavior val2 in Behaviors)
			{
				val2.Initialize(api, val2.properties);
				api.World.FrameProfiler.Mark("initbebehavior-" + val2.GetType());
			}
			return;
		}
	...
	public virtual void CreateBehaviors(Block block, IWorldAccessor worldForResolve)
	{
		...
		foreach (BlockEntityBehaviorType beht in blockEntityBehaviors)
		{
			...
			if (beht.properties == null)
			{
				beht.properties = new JsonObject(new JObject());
			}
	...
	public virtual void ToTreeAttributes(ITreeAttribute tree)
	{
		...
		{
			foreach (KeyValuePair<string, IAttribute> val in missingBlockTree)
			{
