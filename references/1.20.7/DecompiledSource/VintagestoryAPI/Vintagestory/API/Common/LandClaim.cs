...
public class LandClaim
{
	...
	public BlockPos Center
	{
		get
		{
			...
			{
				_ = area2.Center;
				sizeSum += area2.SizeXYZ;
			}
			foreach (Cuboidi area in Areas)
			{
				Vec3i centeri = area.Center;
				Vec3d center = new Vec3d(centeri.X, centeri.Y, centeri.Z);
				centerSum += center * ((double)area.SizeXYZ / (double)sizeSum);
			}
			return new BlockPos((int)(centerSum.X / (double)Areas.Count), (int)(centerSum.Y / (double)Areas.Count), (int)(centerSum.Z / (double)Areas.Count));
		}
	}
	...
	public EnumPlayerAccessResult TestPlayerAccess(IPlayer player, EnumBlockAccessFlags claimFlag)
	{
		...
		if (PermittedPlayerUids.TryGetValue(player.PlayerUID, out var flags) && (flags & claimFlag) > EnumBlockAccessFlags.None)
		{
		...
		{
			if (PermittedPlayerGroupIds.TryGetValue(group.GroupUid, out flags) && (flags & claimFlag) > EnumBlockAccessFlags.None)
			{
