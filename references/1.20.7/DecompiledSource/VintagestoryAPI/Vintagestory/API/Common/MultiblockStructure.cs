...
public class MultiblockStructure
{
	...
	{
		if (!BlockNumbers.TryGetValue(block.Code, out var blockNum))
		{
	...
	public void InitForUse(float rotateYDeg)
	{
		...
		for (int i = 0; i < Offsets.Count; i++)
		{
			...
			Vec4f offsetTf = new Vec4f(offset.X, offset.Y, offset.Z, 0f);
			Vec4f tfedOffset = mat.TransformVector(offsetTf);
			TransformedOffsets.Add(new BlockOffsetAndNumber
			{
				X = (int)Math.Round(tfedOffset.X),
				Y = (int)Math.Round(tfedOffset.Y),
				Z = (int)Math.Round(tfedOffset.Z),
				W = offset.W
			});
		}
	}
