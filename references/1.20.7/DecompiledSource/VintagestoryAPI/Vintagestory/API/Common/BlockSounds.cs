...
public class BlockSounds
{
	...
	public BlockSounds Clone()
	{
		...
		if (ByTool != null)
		{
			sounds.ByTool = new Dictionary<EnumTool, BlockSounds>(ByTool.Count);
			foreach (KeyValuePair<EnumTool, BlockSounds> val in ByTool)
			{
	...
	public AssetLocation GetBreakSound(IPlayer byPlayer)
	{
		...
		if (tool.HasValue)
		{
			return GetBreakSound(tool.Value);
		}
		return Break;
	}
	...
	public AssetLocation GetBreakSound(EnumTool tool)
	{
		if (ByTool != null)
		{
			ByTool.TryGetValue(tool, out var toolSounds);
