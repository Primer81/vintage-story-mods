...
public abstract class InventoryBase : IInventory, IReadOnlyCollection<ItemSlot>, IEnumerable<ItemSlot>, IEnumerable
{
	...
	public virtual void ResolveBlocksOrItems()
	{
		...
		{
			ItemSlot slot = enumerator.Current;
			if (slot.Itemstack != null && !slot.Itemstack.ResolveBlockOrItem(Api.World))
			{
				slot.Itemstack = null;
			}
		}
	...
	public virtual WeightedSlot GetBestSuitedSlot(ItemSlot sourceSlot, ItemStackMoveOperation op = null, List<ItemSlot> skipSlots = null)
	{
		...
		using (IEnumerator<ItemSlot> enumerator = GetEnumerator())
		{
			while (enumerator.MoveNext())
			{
				ItemSlot slot2 = enumerator.Current;
				if ((skipSlots == null || !skipSlots.Contains(slot2)) && slot2.Itemstack != null && slot2.CanTakeFrom(sourceSlot))
				{
					float curWeight2 = GetSuitability(sourceSlot, slot2, isMerge: true);
					if (bestWSlot.slot == null || bestWSlot.weight < curWeight2)
					{
						bestWSlot.slot = slot2;
						bestWSlot.weight = curWeight2;
					}
				}
			}
		}
		...
		{
			ItemSlot slot = enumerator.Current;
			if ((skipSlots == null || !skipSlots.Contains(slot)) && slot.Itemstack == null && slot.CanTakeFrom(sourceSlot))
			{
				float curWeight = GetSuitability(sourceSlot, slot, isMerge: false);
	...
	{
		float extraWeight = ((targetSlot is ItemSlotBackpack && (sourceSlot.Itemstack.Collectible.GetStorageFlags(sourceSlot.Itemstack) & EnumItemStorageFlags.Backpack) > (EnumItemStorageFlags)0) ? 2 : 0);
		return baseWeight + extraWeight + (float)((!isMerge) ? 1 : 3);
	}
	...
	public virtual object ActivateSlot(int slotId, ItemSlot sourceSlot, ref ItemStackMoveOperation op)
	{
		...
		if (op.ShiftDown)
		{
			...
			op.ActingPlayer.InventoryManager.TryTransferAway(sourceSlot, ref op, onlyPlayerInventory: false, shiftClickDebugText);
	...
	public virtual bool TryFlipItemStack(IPlayer owningPlayer, string[] invIds, int[] slotIds, long[] lastChanged)
	{
		...
		return ((InventoryBase)owningPlayer.InventoryManager.GetInventory(invIds[1])).TryFlipItems(slotIds[1], slots[0]) != null;
	}
	...
	public virtual ItemSlot[] SlotsFromTreeAttributes(ITreeAttribute tree, ItemSlot[] slots = null, List<ItemSlot> modifiedSlots = null)
	{
		...
		for (int slotId = 0; slotId < slots.Length; slotId++)
		{
			...
			if (modifiedSlots != null)
			{
				...
				bool num = newstack != null && !newstack.Equals(Api.World, oldstack);
	...
	public void SlotsToTreeAttributes(ItemSlot[] slots, ITreeAttribute tree)
	{
		...
		for (int i = 0; i < slots.Length; i++)
		{
			...
			{
				slotsTree.SetItemstack(i.ToString() ?? "", slots[i].Itemstack.Clone());
			}
		}
	...
	public float InvokeTransitionSpeedDelegates(EnumTransitionType transType, ItemStack stack, float mul)
	{
		if (this.OnAcquireTransitionSpeed != null)
		{
			...
			{
				CustomGetTransitionSpeedMulDelegate dele = (CustomGetTransitionSpeedMulDelegate)invocationList[i];
				mul *= dele(transType, stack, mul);
			}
		}
		return mul;
	}
