...
public class IntDataMap2D
{
	...
	public int GetUnpaddedColorLerped(float x, float z)
	{
		...
		if (index < 0 || index + Size + 1 >= Data.Length)
		{
			throw new IndexOutOfRangeException("MapRegion data, index was " + (index + Size + 1) + " but length was " + Data.Length);
		}
		return GameMath.BiLerpRgbColor(x - (float)ix, z - (float)iz, Data[index], Data[index + 1], Data[index + Size], Data[index + Size + 1]);
	}
	...
	{
		float posXInRegionMap = (float)((double)x / (double)regionSize - (double)(x / regionSize));
		float posZInRegionMap = (float)((double)z / (double)regionSize - (double)(z / regionSize));
	...
	public float GetUnpaddedIntLerped(float x, float z)
	{
		int ix = (int)x;
		int iz = (int)z;
		return GameMath.BiLerp(Data[(iz + TopLeftPadding) * Size + ix + TopLeftPadding], Data[(iz + TopLeftPadding) * Size + ix + 1 + TopLeftPadding], Data[(iz + 1 + TopLeftPadding) * Size + ix + TopLeftPadding], Data[(iz + 1 + TopLeftPadding) * Size + ix + 1 + TopLeftPadding], x - (float)ix, z - (float)iz);
	}
	...
	public float GetIntLerpedCorrectly(float x, float z)
	{
		...
		return GameMath.BiLerp(Data[index], Data[index + 1], Data[index + Size], Data[index + Size + 1], fx, fz);
	}
	...
	public int GetColorLerpedCorrectly(float x, float z)
	{
		int posXLeft = (int)Math.Floor(x - 0.5f);
		int posZLeft = (int)Math.Floor(z - 0.5f);
		float lx = x - ((float)posXLeft + 0.5f);
		float fz = z - ((float)posZLeft + 0.5f);
		int index = (posZLeft + TopLeftPadding) * Size + posXLeft + TopLeftPadding;
		return GameMath.BiLerpRgbColor(lx, fz, Data[index], Data[index + 1], Data[index + Size], Data[index + Size + 1]);
	}
}
