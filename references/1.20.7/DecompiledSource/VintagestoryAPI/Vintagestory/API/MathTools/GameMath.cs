...
public static class GameMath
{
	...
	static GameMath()
	{
		...
		for (int j = 0; j < SIN_COUNT; j++)
		{
			sinValues[j] = (float)Math.Sin(((float)j + 0.5f) / (float)SIN_COUNT * radFull);
			cosValues[j] = (float)Math.Cos(((float)j + 0.5f) / (float)SIN_COUNT * radFull);
		}
		for (int i = 0; i < 360; i += 90)
		{
			sinValues[(int)((float)i * degToIndex) & SIN_MASK] = (float)Math.Sin((double)i * Math.PI / 180.0);
			cosValues[(int)((float)i * degToIndex) & SIN_MASK] = (float)Math.Cos((double)i * Math.PI / 180.0);
		}
	}
	...
	{
		float epsilon = 3.2E-06f;
		float angle = Mod(angleRad, (float)Math.PI * 2f);
		if ((angle + epsilon) % ((float)Math.PI / 2f) <= epsilon * 2f)
		{
			angle = (float)((int)((angle + epsilon) / ((float)Math.PI / 2f)) % 4) * ((float)Math.PI / 2f);
		}
		return angle;
	}
	...
	public static double[,] GenGaussKernel(double sigma = 1.0, int size = 5)
	{
		...
		double mean = (double)size / 2.0;
		double sum = 0.0;
		for (int x2 = 0; x2 < size; x2++)
		{
			for (int y2 = 0; y2 < size; y2++)
			{
				kernel[x2, y2] = Math.Exp(-0.5 * (Math.Pow(((double)x2 - mean) / sigma, 2.0) + Math.Pow(((double)y2 - mean) / sigma, 2.0))) / (Math.PI * 2.0 * sigma * sigma);
				sum += kernel[x2, y2];
			}
		}
		...
		for (int inX = 0; inX < innerSize; inX++)
		{
			for (int inZ = 0; inZ < innerSize; inZ++)
			{
				...
				for (int dz = 0; dz < zoom; dz++)
				{
					...
					{
						int x = inX * zoom + dx;
						result[z * outSize + x] = BiLerpRgbColor((float)dx / (float)zoom, (float)dz / (float)zoom, leftTop, rightTop, leftBottom, rightBottom);
					}
				}
			}
		}
	...
	public static double CPCatmullRomSplineLerp(double t, double[] p, double[] time)
	{
		...
		double L14 = p[2] * (time[3] - t) / (time[3] - time[2]) + p[3] * (t - time[2]) / (time[3] - time[2]);
		double num2 = num * (time[2] - t) / (time[2] - time[0]) + L12 * (t - time[0]) / (time[2] - time[0]);
		double L13 = L12 * (time[3] - t) / (time[3] - time[1]) + L14 * (t - time[1]) / (time[3] - time[1]);
		return num2 * (time[2] - t) / (time[2] - time[1]) + L13 * (t - time[1]) / (time[2] - time[1]);
	}
	...
	public static float TriangleStep(int val, int left, int right)
	{
		...
		return Math.Max(0f, 1f - Math.Abs((float)val - mid) / range);
	}
	...
	public static float TriangleStep(float val, float left, float right)
	{
		...
		float range = (right - left) / 2f;
		return Math.Max(0f, 1f - Math.Abs(val - mid) / range);
	}
	...
	{
		float falloff = Math.Max(smoothingFactor - Math.Abs(a - b), 0f) / smoothingFactor;
	...
	public unsafe static int DotNetStringHash(string text)
	{
		fixed (char* ptr = text)
		{
			...
			int* pint = (int*)ptr;
			int len;
			for (len = text.Length; len > 2; len -= 4)
			{
	...
	{
		double g = 1.618033988749895;
		double a1 = 1.0 / g;
		return (0.5 + a1 * (double)n) % 1.0;
	}
	...
	{
		double g = 1.324717957244746;
		double a1 = 1.0 / g;
		double a2 = 1.0 / (g * g);
		return new Vec2d((0.5 + a1 * (double)n) % 1.0, (0.5 + a2 * (double)n) % 1.0);
	}
	...
	{
		double g = 1.2207440846057596;
		double a1 = 1.0 / g;
		double a2 = 1.0 / (g * g);
		double a3 = 1.0 / (g * g * g);
		return new Vec3d((0.5 + a1 * (double)n) % 1.0, (0.5 + a2 * (double)n) % 1.0, (0.5 + a3 * (double)n) % 1.0);
	}
	...
	public static Vec3d ToEulerAngles(Vec4d q)
	{
		...
		vec3d.X = Math.Atan2(sinr_cosp, cosr_cosp);
		double sinp = 2.0 * (q.W * q.Y - q.Z * q.X);
		vec3d.Y = Math.Asin(sinp);
		double siny_cosp = 2.0 * (q.W * q.Z + q.X * q.Y);
		double cosy_cosp = 1.0 - 2.0 * (q.Y * q.Y + q.Z * q.Z);
		vec3d.Z = Math.Atan2(siny_cosp, cosy_cosp);
		return vec3d;
	}
