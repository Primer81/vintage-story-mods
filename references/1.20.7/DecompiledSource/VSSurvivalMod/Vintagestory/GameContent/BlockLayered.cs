...
public class BlockLayered : Block
{
	...
	{
		int.TryParse(Code.Path.Split('-')[1], out var layer);
		string basecode = CodeWithoutParts(1);
		if (layer < 7)
		{
			return world.BlockAccessor.GetBlock(CodeWithPath(basecode + "-" + (layer + 1)));
		}
	...
	private bool CanLayerStay(IWorldAccessor world, BlockPos pos)
	{
		...
		return world.BlockAccessor.GetBlock(belowPos).CanAttachBlockAt(world.BlockAccessor, this, belowPos, BlockFacing.UP);
	}
