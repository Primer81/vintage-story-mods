...
public class BlockLogSection : Block
{
	...
	{
		base.OnLoaded(api);
		string r = Variant["rotation"];
		string rotation = ((r != null) ? string.Intern(r) : null);
		string t = Variant["segment"];
		string segment = ((t != null) ? string.Intern(t) : null);
	...
	public override bool TryPlaceBlock(IWorldAccessor world, IPlayer byPlayer, ItemStack itemstack, BlockSelection blockSel, ref string failureCode)
	{
		...
		if (rotation == null || (byPlayer != null && byPlayer.Entity.Controls.ShiftKey))
		{
		...
		if (around[normal0] != null && around[normal2] == null)
		{
			...
			reference2 = BlockFacing.FromFirstLetter(around[normal0].LastCodePart(1)[1]).Code.ToLower()[0];
			seg = string.Concat(readOnlySpan, new ReadOnlySpan<char>(in reference2));
		}
		if (around[normal2] != null && around[normal0] == null)
		{
			reference2 = 'n';
			ReadOnlySpan<char> readOnlySpan2 = new ReadOnlySpan<char>(in reference2);
			reference = BlockFacing.FromFirstLetter(around[normal2].LastCodePart(1)[1]).Code.ToLower()[0];
			seg = string.Concat(readOnlySpan2, new ReadOnlySpan<char>(in reference));
		}
		if (around[normal1] != null && around[normal3] == null)
		{
			reference = BlockFacing.FromFirstLetter(around[normal1].LastCodePart(1)[0]).Code.ToLower()[0];
			ReadOnlySpan<char> readOnlySpan3 = new ReadOnlySpan<char>(in reference);
			reference2 = 'w';
			seg = string.Concat(readOnlySpan3, new ReadOnlySpan<char>(in reference2));
		}
		if (around[normal3] != null && around[normal1] == null)
		{
			reference2 = BlockFacing.FromFirstLetter(around[normal3].LastCodePart(1)[0]).Code.ToLower()[0];
			ReadOnlySpan<char> readOnlySpan4 = new ReadOnlySpan<char>(in reference2);
			reference = 'e';
			seg = string.Concat(readOnlySpan4, new ReadOnlySpan<char>(in reference));
		}
		if (seg == null)
		{
	...
	private void SelectSimilarBlocksAround(BlockLogSection[] around, string rotation)
	{
		...
		for (int i = 0; i < 6; i++)
		{
			...
			if (neib != null)
			{
				...
				else if (i != f1 && i != f2 && !neib.IsBarkFace(i))
				{
					around[i] = null;
				}
			}
	...
	private string GetBlocksAround(BlockLogSection[] around, IBlockAccessor blockAccessor, BlockPos pos)
	{
		...
		if (blockAccessor.GetBlock(pos.North()) is BlockLogSection i)
		{
			around[0] = i;
			UpdateAny(i.LastCodePart(), any, anyCount);
		}
