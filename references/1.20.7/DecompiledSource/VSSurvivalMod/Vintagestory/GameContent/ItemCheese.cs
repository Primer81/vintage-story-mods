...
public class ItemCheese : Item
{
	...
	public override void OnHeldInteractStart(ItemSlot slot, EntityAgent byEntity, BlockSelection blockSel, EntitySelection entitySel, bool firstEvent, ref EnumHandHandling handling)
	{
		if (byEntity.Controls.ShiftKey && blockSel != null)
		{
			...
			{
				if (api.World.BlockAccessor.GetBlockEntity(targetPos) is BECheese)
				{
					slot.TakeOut(1);
					slot.MarkDirty();
				}
				api.World.PlaySoundAt(placeblock.Sounds.Place, (double)targetPos.X + 0.5, targetPos.InternalY, (double)targetPos.Z + 0.5, byPlayer);
