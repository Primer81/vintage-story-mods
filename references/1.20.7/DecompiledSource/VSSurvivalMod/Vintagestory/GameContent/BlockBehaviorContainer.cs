...
public class BlockBehaviorContainer : BlockBehavior
{
	...
	public override bool OnBlockInteractStart(IWorldAccessor world, IPlayer byPlayer, BlockSelection blockSel, ref EnumHandling handling)
	{
		...
		{
			return ((BlockEntityOpenableContainer)entity).OnPlayerRightClick(byPlayer, blockSel);
		}
		return false;
	}
	...
	public override void Activate(IWorldAccessor world, Caller caller, BlockSelection blockSel, ITreeAttribute activationArgs, ref EnumHandling handled)
	{
		...
		BlockEntity entity = world.BlockAccessor.GetBlockEntity(blockSel.Position);
		int timeOut = (int)activationArgs.TryGetLong("close").GetValueOrDefault(2000L);
		BlockEntityOpenableContainer container = entity as BlockEntityOpenableContainer;
		if (container != null)
		{
			byte[] open = SerializerUtil.Serialize(new OpenContainerLidPacket(caller.Entity.EntityId, opened: true));
			((ICoreServerAPI)world.Api).Network.BroadcastBlockEntityPacket(blockSel.Position, 5001, open);
			...
			{
				byte[] data = SerializerUtil.Serialize(new OpenContainerLidPacket(caller.Entity.EntityId, opened: false));
