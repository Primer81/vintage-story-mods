...
public class BlockCharcoalPile : BlockLayeredSlowDig
{
	public override bool CanAcceptFallOnto(IWorldAccessor world, BlockPos pos, Block fallingBlock, TreeAttribute blockEntityAttributes)
	{
		...
		{
			if (world.BlockAccessor.GetMostSolidBlock(pos) is BlockCharcoalPile ourBlock)
			{
				return ourBlock.CountLayers() < 8;
			}
			return false;
		}
		return false;
	...
	public override bool OnFallOnto(IWorldAccessor world, BlockPos pos, Block block, TreeAttribute blockEntityAttributes)
	{
		...
		if (uBlock != null && nBlock != null && nBlock.CountLayers() < 8)
		{
			...
			if (uBlock != null)
			{
				...
				{
					aboveBlock.OnFallOnto(world, pos, uBlock, blockEntityAttributes);
				}
			}
	...
	public override void OnBlockBroken(IWorldAccessor world, BlockPos pos, IPlayer byPlayer, float dropQuantityMultiplier = 1f)
	{
		...
		while (abovePos.Y < world.BlockAccessor.MapSizeY)
		{
			...
			if (aboveBlock.FirstCodePart() != block.FirstCodePart())
			{
				break;
			}
			abovePos.Up();
		}
		if (abovePos == pos.UpCopy() || aboveBlock == null || (byPlayer != null && byPlayer.WorldData.CurrentGameMode == EnumGameMode.Creative))
		{
			base.OnBlockBroken(world, pos, byPlayer, 1f);
			world.BlockAccessor.TriggerNeighbourBlockUpdate(pos);
		}
		else if (aboveBlock.FirstCodePart() == block.FirstCodePart())
		{
			aboveBlock.OnBlockBroken(world, abovePos, byPlayer);
		}
		else
		{
			BlockPos topPos = abovePos.DownCopy();
			world.BlockAccessor.GetBlock(topPos).OnBlockBroken(world, topPos, byPlayer);
		}
	}
