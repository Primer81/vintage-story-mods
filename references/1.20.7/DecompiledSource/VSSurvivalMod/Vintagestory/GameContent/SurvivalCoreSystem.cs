...
public class SurvivalCoreSystem : ModSystem
{
	...
	{
		GameVersion.EnsureEqualVersionOrKillExecutable(api, FileVersionInfo.GetVersionInfo(Assembly.GetExecutingAssembly().Location).FileVersion, "1.20.7", "VSSurvivalMod");
		this.api = api;
	...
	public override void AssetsLoaded(ICoreAPI api)
	{
		...
		foreach (MetalProperty metals in api.Assets.GetMany<MetalProperty>(api.Logger, "worldproperties/block/metal.json").Values)
		{
			for (int i = 0; i < metals.Variants.Length; i++)
			{
				MetalPropertyVariant metal = metals.Variants[i];
	...
	public SolarSphericalCoords GetSolarSphericalCoords(double posX, double posZ, float yearRel, float dayRel)
	{
		...
		float hourAngle = (float)Math.PI * 2f * (dayRel - 0.5f);
		float declination = (0f - EarthAxialTilt) * GameMath.Cos((float)Math.PI * 2f * (yearRel + 0.02739726f));
		float zenithAngle = (float)Math.Acos(GameMath.Sin(latitude * ((float)Math.PI / 2f)) * GameMath.Sin(declination) + GameMath.Cos(latitude * ((float)Math.PI / 2f)) * GameMath.Cos(declination) * GameMath.Cos(hourAngle));
		float azimuthAngle = (float)Math.Acos(GameMath.Clamp((GameMath.Sin(latitude * ((float)Math.PI / 2f)) * GameMath.Cos(zenithAngle) - GameMath.Sin(declination)) / (GameMath.Cos(latitude * ((float)Math.PI / 2f)) * GameMath.Sin(zenithAngle) + 1E-07f), -1f, 1f));
		azimuthAngle = ((float)Math.PI + (float)Math.Sign(hourAngle) * azimuthAngle) % ((float)Math.PI * 2f);
		return new SolarSphericalCoords((float)Math.PI * 2f - zenithAngle, (float)Math.PI * 2f - azimuthAngle);
	}
	...
	private void applyConfig()
	{
		...
		if (api.Side == EnumAppSide.Server)
		{
			...
			{
				if (obj.Tool.HasValue)
				{
					obj.Durability = (int)((float)obj.Durability * config.ToolDurabilityModifier);
				}
			}
		}
		applySeasonConfig();
	...
	private void applySeasonConfig()
	{
		...
		{
			string @string = api.World.Config.GetString("seasons");
			if (@string == "spring")
			{
