...
public class BlockMetalPartPile : Block
{
	...
	public override ItemStack[] GetDrops(IWorldAccessor world, BlockPos pos, IPlayer byPlayer, float dropQuantityMultiplier = 1f)
	{
		...
		int index = ((!(rand.NextDouble() < entry.ScrapChance)) ? 1 : 0);
		float chance = (byPlayer?.Entity.Stats.GetBlended("rustyGearDropRate") ?? 0f) - 1f;
		if (chance > 0f && rand.NextDouble() < (double)chance)
		{
			index = 2;
		}
		...
		foreach (KeyValuePair<int, double> val in (index == 0) ? entry.ScrapQuantityChances : entry.PartQuantityChances)
		{
			rndVal -= val.Value;
			if (rndVal <= 0.0)
			{
