...
public class BlockTorch : BlockGroundAndSideAttachable, IIgnitable
{
	...
	public override bool OnHeldInteractStep(float secondsUsed, ItemSlot slot, EntityAgent byEntity, BlockSelection blockSel, EntitySelection entitySel)
	{
		if (isExtinct && blockSel != null && byEntity.World.BlockAccessor.GetBlock(blockSel.Position) is IIgnitable ign)
		{
			...
			switch (ign.OnTryIgniteStack(byEntity, blockSel.Position, slot, secondsUsed))
			{
			...
			case EnumIgniteState.IgniteNow:
			{
				...
				ItemStack stack = new ItemStack(byEntity.World.GetBlock(CodeWithVariant("state", "lit")));
				if (slot.StackSize == 1)
				{
	...
	public override void OnAttackingWith(IWorldAccessor world, Entity byEntity, Entity attackedEntity, ItemSlot itemslot)
	{
		...
		if (!IsExtinct && attackedEntity != null && byEntity.World.Side == EnumAppSide.Server && api.World.Rand.NextDouble() < 0.1 + (double)stormstr)
		{
	...
	{
		Block liquid = blockAccessor.GetBlock(pos, 2);
		if (liquid.IsLiquid() && liquid.LiquidLevel > 3)
		{
			Block block = api.World.GetBlock(CodeWithVariant("state", "burnedout"));
			if (block != null)
			{
				api.World.BlockAccessor.SetBlock(block.Id, pos);
			}
