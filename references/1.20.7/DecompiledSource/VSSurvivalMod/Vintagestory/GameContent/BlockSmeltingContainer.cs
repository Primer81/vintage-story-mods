...
public class BlockSmeltingContainer : Block
{
	...
	public override float GetMeltingDuration(IWorldAccessor world, ISlotProvider cookingSlotsProvider, ItemSlot inputSlot)
	{
		...
		for (int i = 0; i < stacks.Length; i++)
		{
			if (stacks[i]?.Collectible?.CombustibleProps != null)
			{
				...
				duration += singleDuration * (float)stacks[i].StackSize / (float)stacks[i].Collectible.CombustibleProps.SmeltedRatio;
			}
		}
		return duration;
	}
	...
	public override bool CanSmelt(IWorldAccessor world, ISlotProvider cookingSlotsProvider, ItemStack inputStack, ItemStack outputStack)
	{
		...
		return GetSingleSmeltableStack(stacks) != null;
	}
	...
	public override void DoSmelt(IWorldAccessor world, ISlotProvider cookingSlotsProvider, ItemSlot inputSlot, ItemSlot outputSlot)
	{
		...
		if (alloy != null)
		{
			...
			int units = (int)Math.Round(alloy.GetTotalOutputQuantity(stacks) * 100.0, 0);
		...
		if (match != null)
		{
			((BlockSmeltedContainer)block).SetContents(outputStack, match.output, (int)Math.Round(match.stackSize * 100.0, 0));
			outputStack.Collectible.SetTemperature(world, outputStack, GetIngredientsTemperature(world, stacks));
			outputSlot.Itemstack = outputStack;
			inputSlot.Itemstack = null;
	...
	public string GetOutputText(IWorldAccessor world, ISlotProvider cookingSlotsProvider, ItemSlot inputSlot)
	{
		...
		if (inputSlot.Itemstack.Collectible is BlockSmeltingContainer)
		{
			...
			if (alloy != null)
			{
				...
				return Lang.Get("Will create {0} units of {1}", (int)Math.Round(quantity * 100.0, 0), GetMetal(alloy.Output.ResolvedItemstack));
			}
			MatchedSmeltableStack match = GetSingleSmeltableStack(stacks);
			if (match != null)
			{
				return Lang.Get("Will create {0} units of {1}", (int)Math.Round(match.stackSize * 100.0, 0), GetMetal(match.output));
			}
			return null;
		}
		return null;
	...
	public static MatchedSmeltableStack GetSingleSmeltableStack(ItemStack[] stacks)
	{
		...
		for (int i = 0; i < stacks.Length; i++)
		{
			...
			{
				stackSize *= (double)stack.Collectible.CombustibleProps.SmeltedStack.StackSize;
				stackSize /= (double)stack.Collectible.CombustibleProps.SmeltedRatio;
