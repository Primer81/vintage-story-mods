...
public class BlockArchimedesScrew : BlockMPBase, IBlockItemFlow
{
	...
	public override bool TryPlaceBlock(IWorldAccessor world, IPlayer byPlayer, ItemStack itemstack, BlockSelection blockSel, ref string failureCode)
	{
		...
		foreach (BlockFacing face in vERTICALS)
		{
			BlockPos pos = blockSel.Position.AddCopy(face);
			if (world.BlockAccessor.GetBlock(pos) is IMechanicalPowerBlock block && block.HasMechPowerConnectorAt(world, pos, face.Opposite) && blockToPlace.DoPlaceBlock(world, byPlayer, blockSel, itemstack))
			{
	...
	public override void OnNeighbourBlockChange(IWorldAccessor world, BlockPos pos, BlockPos neibpos)
	{
		...
		if (bemp != null && !bemp.IsAttachedToBlock())
		{
			...
			foreach (BlockFacing face in vERTICALS)
			{
				if (world.BlockAccessor.GetBlock(pos.AddCopy(face)) is BlockAngledGears blockagears && blockagears.Facings.Contains(face.Opposite) && blockagears.Facings.Length == 1)
				{
