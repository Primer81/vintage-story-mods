...
public class BlockBehaviorLadder : BlockBehavior
{
	...
	public override bool TryPlaceBlock(IWorldAccessor world, IPlayer byPlayer, ItemStack itemstack, BlockSelection blockSel, ref EnumHandling handled, ref string failureCode)
	{
		...
		if (belowBlock.GetBehavior<BlockBehaviorLadder>()?.LadderType == LadderType && HasSupport(belowBlock, world.BlockAccessor, pos) && belowBlock.CanPlaceBlock(world, byPlayer, blockSel, ref failureCode))
		{
			belowBlock.DoPlaceBlock(world, byPlayer, blockSel, itemstack);
			return true;
		}
		if (blockSel.HitPosition.Y < 0.5 && TryStackDown(byPlayer, world, aimedAtPos, blockSel.Face, itemstack))
		{
			return true;
		}
	...
	protected bool TryStackUp(IPlayer byPlayer, IWorldAccessor world, BlockPos pos, BlockFacing face, ItemStack itemstack)
	{
		...
		if (ladderBlock.GetBehavior<BlockBehaviorLadder>()?.LadderType != LadderType)
		{
			return false;
		}
		...
		while (abovePos.Y < world.BlockAccessor.MapSizeY)
		{
			...
			if (aboveBlock.FirstCodePart() != ownFirstCodePart)
			{
				break;
			}
			abovePos.Up();
		...
		string useless = "";
		if (aboveBlock == null || aboveBlock.FirstCodePart() == ownFirstCodePart)
		{
			return false;
		}
	...
	protected bool TryCollectLowest(IPlayer byPlayer, IWorldAccessor world, BlockPos pos)
	{
		...
		BlockPos belowPos = pos.DownCopy();
		while (belowPos.Y > 0 && !(world.BlockAccessor.GetBlock(belowPos).FirstCodePart() != ownFirstCodePart))
		{
			belowPos.Down();
		}
		belowPos.Up();
		Block collectBlock = world.BlockAccessor.GetBlock(belowPos);
		BlockBehaviorLadder bh = collectBlock.GetBehavior<BlockBehaviorLadder>();
		...
		world.PlaySoundAt(collectBlock.Sounds.Break, pos, 0.0, byPlayer);
		if (stacks.Length != 0 && !byPlayer.InventoryManager.TryGiveItemstack(stacks[0], slotNotifyEffect: true))
		{
	...
	public bool HasSupportUp(Block forBlock, IBlockAccessor blockAccess, BlockPos pos)
	{
		...
		if (!SideSolid(blockAccess, pos, ownFacing) && !SideSolid(blockAccess, upPos, BlockFacing.UP))
		{
			if (pos.Y < blockAccess.MapSizeY - 1 && blockAccess.GetBlock(upPos) == forBlock)
			{
				return HasSupportUp(forBlock, blockAccess, upPos);
			}
			return false;
		}
		return true;
	...
	public bool HasSupportDown(Block forBlock, IBlockAccessor blockAccess, BlockPos pos)
	{
		...
		if (!SideSolid(blockAccess, pos, ownFacing) && !SideSolid(blockAccess, downPos, BlockFacing.DOWN))
		{
			if (pos.Y > 0 && blockAccess.GetBlock(downPos) == forBlock)
			{
				return HasSupportDown(forBlock, blockAccess, downPos);
			}
			return false;
		}
		return true;
	...
	public bool HasSupport(Block forBlock, IBlockAccessor blockAccess, BlockPos pos)
	{
		...
		BlockPos downPos = pos.DownCopy();
		BlockPos upPos = pos.UpCopy();
		if (!SideSolid(blockAccess, pos, ownFacing) && (isFlexible || !SideSolid(blockAccess, pos, BlockFacing.DOWN)) && !SideSolid(blockAccess, pos, BlockFacing.UP) && (pos.Y >= blockAccess.MapSizeY - 1 || blockAccess.GetBlock(upPos) != forBlock || !HasSupportUp(forBlock, blockAccess, upPos)))
		{
			if (!isFlexible && pos.Y > 0 && blockAccess.GetBlock(downPos) == forBlock)
			{
				return HasSupportDown(forBlock, blockAccess, downPos);
			}
			return false;
	...
	public bool SideSolid(IBlockAccessor blockAccess, BlockPos pos, BlockFacing facing)
	{
		...
		Cuboidi upHalf = new Cuboidi(14, 0, 0, 15, 7, 15).RotatedCopy(0, 90 * facing.HorizontalAngleIndex, 0, new Vec3d(7.5, 0.0, 7.5));
		if (neibBlock.CanAttachBlockAt(blockAccess, neibBlock, neibPos, facing.Opposite, upHalf))
		{
			return true;
		}
		Cuboidi downHalf = new Cuboidi(14, 8, 0, 15, 15, 15).RotatedCopy(0, 90 * facing.HorizontalAngleIndex, 0, new Vec3d(7.5, 0.0, 7.5));
		if (neibBlock.CanAttachBlockAt(blockAccess, neibBlock, neibPos, facing.Opposite, downHalf))
		{
			return true;
		}
		return false;
