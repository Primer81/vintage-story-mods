...
public class ItemCandle : Item
{
	public override void OnHeldInteractStart(ItemSlot slot, EntityAgent byEntity, BlockSelection blockSel, EntitySelection entitySel, bool firstEvent, ref EnumHandHandling handHandling)
	{
		...
		AssetLocation loc = new AssetLocation(Attributes["blockfirstcodepart"].AsString());
		...
		if (targetedBlock.FirstCodePart() == firstcodepart)
		{
			...
			nextblock = world.GetBlock(targetedBlock.CodeWithPart((stage + 1).ToString() ?? "", 1));
		...
		else
		{
			...
			if (!world.BlockAccessor.GetBlock(offsetedPos).IsReplacableBy(nextblock) || !world.BlockAccessor.GetBlock(belowPos).CanAttachBlockAt(world.BlockAccessor, nextblock, belowPos, BlockFacing.UP, new Cuboidi(1, 14, 1, 14, 15, 14)))
			{
				return;
			}
