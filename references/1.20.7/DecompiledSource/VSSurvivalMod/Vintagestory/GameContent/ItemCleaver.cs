...
public class ItemCleaver : Item
{
	...
	private void Bhaa_OnBeginHitEntity(EntityAgent byEntity, ref EnumHandling handling)
	{
		...
		ItemSlot slot = (byEntity as EntityPlayer)?.Player.InventoryManager.ActiveHotbarSlot;
		if (entitySel != null && !(entitySel.Entity is EntityPlayer))
		{
			int @int = entitySel.Entity.WatchedAttributes.GetInt("generation");
			slot?.Itemstack?.Collectible.DamageItem(byEntity.World, byEntity, slot);
			float slaughterChance = (float)@int / 3f;
			float dmg = 9999f;
			if (api.World.Rand.NextDouble() > (double)slaughterChance)
			{
				dmg = 1f;
			}
