...
public class BlockBehaviorHorizontalOrientable : BlockBehavior
{
	...
	public override bool TryPlaceBlock(IWorldAccessor world, IPlayer byPlayer, ItemStack itemstack, BlockSelection blockSel, ref EnumHandling handling, ref string failureCode)
	{
		...
		{
			throw new NullReferenceException(string.Concat("Unable to to find a rotated block with code ", blockCode, ", you're maybe missing the side variant group of have a dash in your block code"));
		}
	...
	{
		handled = EnumHandling.PreventDefault;
		int rotatedIndex = GameMath.Mod(BlockFacing.FromCode(block.Variant[variantCode]).HorizontalAngleIndex - angle / 90, 4);
