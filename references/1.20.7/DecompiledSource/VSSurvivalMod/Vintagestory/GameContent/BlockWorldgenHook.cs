...
public class BlockWorldgenHook : Block
{
	...
	private TextCommandResult onHookConfig(TextCommandCallingArgs args)
	{
		...
		if (!(sapi.World.BlockAccessor.GetBlockEntity(target) is BlockEntityWorldgenHook bewh))
		{
			return TextCommandResult.Error("No WorldgenHook block found at position " + target);
		}
		if (hook == null)
		{
			return TextCommandResult.Success("Hook is currently: \"" + bewh.GetHook() + "\"");
		}
