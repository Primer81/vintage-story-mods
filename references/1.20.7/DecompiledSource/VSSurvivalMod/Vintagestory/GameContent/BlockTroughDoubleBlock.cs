...
public class BlockTroughDoubleBlock : BlockTroughBase
{
	...
	public override bool TryPlaceBlock(IWorldAccessor world, IPlayer byPlayer, ItemStack itemstack, BlockSelection blockSel, ref string failureCode)
	{
		if (CanPlaceBlock(world, byPlayer, blockSel, ref failureCode))
		{
			...
			world.BlockAccessor.GetBlock(CodeWithParts("head", code)).DoPlaceBlock(world, byPlayer, new BlockSelection
			{
				Position = secondPos,
				Face = blockSel.Face
			}, itemstack);
	...
	public override bool OnBlockInteractStart(IWorldAccessor world, IPlayer byPlayer, BlockSelection blockSel)
	{
		if (blockSel != null)
		{
			...
			bool ok = (world.BlockAccessor.GetBlockEntity(pos + RootOffset) as BlockEntityTrough)?.OnInteract(byPlayer, blockSel) ?? false;
