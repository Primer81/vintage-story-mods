...
internal class MapLayerFuzzyZoom : MapLayerBase
{
	...
	public override int[] GenLayer(int xPos, int zPos, int xSize, int zSize)
	{
		...
		for (int z2 = 0; z2 < newZSize - 1; z2++)
		{
			int index = (z2 << 1) * outXsize;
			for (int x = 0; x < newXSize - 1; x++)
			{
				InitPositionSeed(xCoord + x, zCoord + z2);
				int valTopLeft = parentInts[x + z2 * newXSize];
				int valTopRight = parentInts[x + 1 + z2 * newXSize];
				int valBottomLeft = parentInts[x + (z2 + 1) * newXSize];
				int valBottomRight = parentInts[x + 1 + (z2 + 1) * newXSize];
