...
public class MapLayerDoublePerlin : MapLayerBase
{
	...
	public override int[] GenLayer(int xCoord, int zCoord, int sizeX, int sizeZ)
	{
		...
		for (int z = 0; z < sizeZ; z++)
		{
			...
			{
				outData[z * sizeX + x] = (int)GameMath.Clamp((double)multiplier * noisegenX.Noise(xCoord + x, zCoord + z), 0.0, 255.0) | ((int)GameMath.Clamp((double)multiplier * noisegenY.Noise(xCoord + x, zCoord + z), 0.0, 255.0) << 8);
			}
		}
		return outData;
	}
