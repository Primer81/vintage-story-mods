...
public class MapLayerPerlinWobble : MapLayerTransformBase
{
	...
	public override int[] GenLayer(int xCoord, int zCoord, int sizeX, int sizeZ)
	{
		...
		for (int y = 0; y < sizeZ; y++)
		{
			...
			{
				int offsetX = (int)((double)intensity * noisegenX.Noise(xCoord + x + margin, zCoord + y + margin));
				int offsetY = (int)((double)intensity * noisegenY.Noise(xCoord + x + margin, zCoord + y + margin));
