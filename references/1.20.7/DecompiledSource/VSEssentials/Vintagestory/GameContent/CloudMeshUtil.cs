...
public class CloudMeshUtil : CubeMeshUtil
{
	public static MeshData GetCubeModelDataForClouds(float scaleH, float scaleV, Vec3f translate)
	{
		...
		modeldata.xyz = new float[72];
		modeldata.Normals = new int[24];
		for (int i = 0; i < 24; i++)
		{
			modeldata.xyz[3 * i] = (float)CubeMeshUtil.CubeVertices[3 * i] * scaleH + translate.X;
			modeldata.xyz[3 * i + 1] = (float)CubeMeshUtil.CubeVertices[3 * i + 1] * scaleV + translate.Y;
			modeldata.xyz[3 * i + 2] = (float)CubeMeshUtil.CubeVertices[3 * i + 2] * scaleH + translate.Z;
		}
		modeldata.SetVerticesCount(24);
		modeldata.SetIndices(CubeMeshUtil.CubeVertexIndices);
		modeldata.SetIndicesCount(36);
		return modeldata;
	}
