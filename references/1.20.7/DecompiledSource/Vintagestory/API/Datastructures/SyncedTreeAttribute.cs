...
public class SyncedTreeAttribute : TreeAttribute
{
    ...
    //
    // Summary:
    //     Marks the whole attribute tree as dirty, so that it will be resent to all connected
    //     clients. Does not trigger modified listeners (because it makes no sense and breaks
    //     things)
    ...
    //
    // Summary:
    //     Marks part of the attribute tree as dirty, allowing for a partial update of the
    //     attribute tree. Has no effect it the whole tree is already marked dirty. If more
    //     than 5 paths are marked dirty it will wipe the list of dirty paths and just marked
    //     the whole tree as dirty
    //
    // Parameters:
    //   path:
    ...
    public void GetDirtyPathData(FastMemoryStream ms, out string[] paths, out byte[][] dirtydata)
    {
        ...
        for (int i = 0; i < paths.Length; i++)
        {
            if (paths[i] != null)
            {
                IAttribute attributeByPath = GetAttributeByPath(paths[i]);
