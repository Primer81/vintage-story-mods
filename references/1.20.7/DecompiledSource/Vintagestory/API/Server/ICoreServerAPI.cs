...
//
// Summary:
//     The core api implemented by the server. The main interface for accessing the
//     server. Contains all sub components and some miscellaneous methods.
public interface ICoreServerAPI : ICoreAPI, ICoreAPICommon
{
    ...
    //
    // Summary:
    //     World-agnostic player data. You can query this information even when the player
    //     is offline
    ...
    //
    // Summary:
    //     Shows a vibrating red text in the players screen. If text is null the client
    ...
    //
    //   langparams:
    //     If text is null, these are the arguments passed into the Language translation
    //     tool
    void SendIngameError(IServerPlayer player, string errorCode, string text = null, params object[] langparams);

    //
    // Summary:
    //     Shows a discovery text on the players screen. If text is null the client will
    //     try to find a language entry using supplied code prefixed with 'ingamediscovery-'
    //     (which is recommended so that the errors are translated to the users local language)
    //
    //
    // Parameters:
    ...
    //
    // Summary:
    //     Injects a message or command into the server console input processing system.
    //     This lets you run commands or chat as Admin.
    //
    // Parameters:
    //   message:
    ...
    //
    // Summary:
    //     Register a new Block. Must happen before server runphase LoadGame. These are
    //     sent to the client during connect, so only need to register them on the server
    //     side.
    //
    // Parameters:
    //   block:
    ...
    //
    // Summary:
    //     Registers a new crafting recipe. These are sent to the client during connect,
    //     so only need to register them on the server side.
    //
    // Parameters:
    //   recipe:
    ...
    //
    // Summary:
    //     Registers a chat command. When registered on the client you access the command
    //     by prefixing a dot (.), on the server it's a slash (/)
    //
    // Parameters:
    //   chatcommand:
    [Obsolete("Use ChatCommand subapi instead")]
    ...
    //
    // Summary:
    //     For internal use: used to remap block and item Ids, as soon as assets are loaded
    //     from disk, before recipes etc. are loaded or anything else which may occur in
    //     modsystem AssetsLoaded() methods
