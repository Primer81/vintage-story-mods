...
public interface IInventory : IReadOnlyCollection<ItemSlot>, IEnumerable<ItemSlot>, IEnumerable
{
    ...
    //
    // Summary:
    //     Wether or not to remove the inventory from the players inventory list upon closing
    //     it
    ...
    //
    // Summary:
    //     Milliseconds since server startup when the inventory was last changed (not used
    //     currently)
    ...
    //
    // Summary:
    //     Returns the best suited slot to hold the item from the source slot. Attached
    //     is also a weight, indicating how well the item is suited for it. If no suitable
    //     slot was found, the weight will be 0 and the slot will be null. A higher weight
    //     means the slot is better suited to hold the item. This method does not check
    //     if the player is actually allowed to access or modify this inventory. Weight
    //     will be 1 for a default slot that is empty Weight will be 2 for a default slot
    //     that can take one or more items from the source slot Weight could be 10 for an
    //     empty armor slot and the source slot contains an armor itemtack
    //
    // Parameters:
    //   sourceSlot:
    //
