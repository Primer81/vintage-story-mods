...
public interface IGameCalendar
{
    ...
    //
    // Summary:
    //     Assigned by the survival mod. The calendar uses this method to determine the
    //     solar altitude at given location and time. If not set, the calendar uses a default
    //     value of about 0.75
    SolarSphericalCoordsDelegate OnGetSolarSphericalCoords { get; set; }

    //
    // Summary:
    //     Assigned by the survival mod. Must return the latitude for given position. If
    //     not set, the calendar uses a default value of 0.5
    //     -1 for south pole, 0 for equater, 1 for north pole
    GetLatitudeDelegate OnGetLatitude { get; set; }

    //
    // Summary:
    //     This acts as a multiplier on how much faster an ingame second passes by compared
    //     to a real life second. Affects physics, like the motion speed of waving grass.
    //     The default is 60, hence per default a day lasts 24 minutes, but it's also multiplied
    //     by CalendarSpeedMul which is 0.5 by default so the end result is 48 minutes per
    //     day This is the sum of all modifiers
    ...
    //
    // Summary:
    //     A multiplier thats applied to the progression of the calendar. Set this to 0.1
    //     and a day will last 10 times longer, does not affect physics.
    ...
    //
    // Summary:
    //     Can be used to adjust apparent time of day and season for rendering, e.g. to
    //     create a series of timelapse images; restore to 0 when done
    float Timelapse { get; set; }

    //
    // Summary:
    //     Retrieve the current daylight strength at given coordinates, the values are roughly
    //     beween 0 and 1.2f
    //
    // Parameters:
    //   x:
    ...
    //
    // Summary:
    //     Retrieve the current daylight strength at given coordinates. The Y-Component
    //     is ignored
    //
    // Parameters:
    //   pos:
    ...
    //
    // Summary:
    //     If non-null, will override the value retrieved by GetSeason(). Set to null to
    //     have seasons progress normally again.
    //
    // Parameters:
    //   seasonRel:
