...
public interface IParticlePropertiesProvider
{
    //
    // Summary:
    //     If true, will be be spawned in the async particle system, which does not affect
    //     main game performance, recommended for large quantities of particles, slightly
    //     less optimal for particles that spawn very often
    bool Async { get; }
    ...
    //
    // Summary:
    //     Whether the particle should despawn when in contact with air (e.g. for water
    //     bubbles)
    bool DieInAir { get; }
    ...
    //
    // Summary:
    //     How many particles should spawn? For every particle spawned, all of belows methods
    //     are called once. E.g. if quantity is 10, GetPos(), GetVelocity(),... is called
    //     10 times.
    float Quantity { get; }
    ...
    //
    // Summary:
    //     If this particle emits light, this is its RGBA. Does not actually brighten the
    //     scene around it, but prevents incorrect lighting of particles when everything
    //     else around it is dark
    int LightEmission { get; }

    //
    // Summary:
    //     Return null or 1 if opacity should remain unchanged over time. lifetimeLeft is
    ...
    //
    // Summary:
    //     See also Vintagestory.API.Common.IParticlePropertiesProvider.VertexFlags
    int VertexFlags { get; }

    //
    // Summary:
    //     If true, a particle will restore it's initial velocity once it's obstruction
    //     has been cleared e.g. Smokes will start flying upwards again if is currently
    //     stuck under a block and the block is removed
    ...
    //
    // Summary:
    //     Determines the interval of time that must elapse during it's parent particle's
    //     lifetime before this one will spawn. This is only honored if this particle is
    //     defined as a secondary particle.
    float SecondarySpawnInterval { get; }

    //
    // Summary:
    //     The secondary particle properties. Secondary particles are particles that are
    //     emitted from an in-flight particle.
    ...
    //
    // Summary:
    //     Called just before a new particle is being created. You can use this to produce
    //     e.g. alternating kinds of particles
    void BeginParticle();
    ...
    //
    // Summary:
    //     Updates instance related state for secondary particles based on the given parent
    //     particle instance
    //
    // Parameters:
    //   particleInstance:
