...
public class PlayerHeadController : EntityHeadController
{
    ...
    public override void OnFrame(float dt)
    {
        ...
        if (turnOpposite)
        {
            if (Math.Abs(num) < MathF.PI * 9f / 20f)
            {
                turnOpposite = false;
            }
            else
        ...
        if (!flag && coreClientAPI.Input.MouseGrabbed)
        {
            ...
            entity.Pos.HeadPitch = GameMath.Clamp((entity.Pos.Pitch - MathF.PI) * 0.75f, -1.2f, 1.2f);
        }
        ...
        if (player?.Entity == null || enumMountAngleMode == EnumMountAngleMode.Fixate || enumMountAngleMode == EnumMountAngleMode.FixateYaw || cameraMode == EnumCameraMode.Overhead)
        {
            if (coreClientAPI.Input.MouseGrabbed)
            {
                ...
                if (flag)
                {
                    ...
                    float num3 = MathF.PI + num2;
                    float num4 = GameMath.Clamp(0f - entity.Pos.Pitch - MathF.PI + MathF.PI * 2f, -1f, 0.8f);
                    if (num3 > MathF.PI)
                    {
                        num3 -= MathF.PI * 2f;
                    }
                    ...
                    {
                        num3 = 0f;
                        entity.Pos.HeadPitch += (GameMath.Clamp((entity.Pos.Pitch - MathF.PI) * 0.75f, -1.2f, 1.2f) - entity.Pos.HeadPitch) * dt * 6f;
                    }
                    else
                    {
        ...
        else
        {
            ...
            if (obj != null && obj.Entity.Alive)
            {
                ...
                if (player.PlayerUID == coreClientAPI.World.Player.PlayerUID && !coreClientAPI.Settings.Bool["immersiveFpMode"] && cameraMode != 0)
                {
