...
public class AnimationFrame
{
    ...
    public void FinalizeMatrices(Dictionary<int, AnimationJoint> jointsById)
    {
        ...
        {
            float[] array = Mat4f.CloneIt(animTransforms[i]);
            if (jointsById.ContainsKey(i))
            {
                Mat4f.Mul(array, array, jointsById[i].Element.inverseModelTransform);
            }

            for (int j = 0; j < 16; j++)
            {
                transformationMatrices[num++] = array[j];
            }
        }
