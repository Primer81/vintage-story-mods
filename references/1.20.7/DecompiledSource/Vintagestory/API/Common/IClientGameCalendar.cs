...
public interface IClientGameCalendar : IGameCalendar
{
    ...
    //
    // Summary:
    //     A horizontal offset that is applied when reading the sky glow color at the players
    //     current location. Creates a greater variety of sunsets. Changes to a different
    //     value once per day (during midnight)
    float SunsetMod { get; }

    //
    // Summary:
    //     Returns a value between 0 (no sunlight) and 1 (full sunlight) at the players
    //     current location
