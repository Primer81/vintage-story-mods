...
public class DamagedPerceptionEffect : PerceptionEffect
{
    ...
    private void OnHurt()
    {
        ...
        if (strength != 0f && capi.World.Player.Entity.RemainingActivityTime("invulnerable") > 0)
        {
            ...
            float @float = entity.WatchedAttributes.GetFloat("onHurtDir");
            if (@float < -99f)
            {
            ...
            float num = GameMath.AngleRadDistance(entity.Pos.Yaw - MathF.PI / 2f, @float);
            capi.Render.ShaderUniforms.DamageVignettingSide = GameMath.Clamp(num / (MathF.PI / 2f), -1f, 1f);
        }
    }
    ...
    private float CalculateHealthThreshold()
    {
        ...
        float valueOrDefault = (treeAttribute?.GetFloat("currenthealth") / treeAttribute?.GetFloat("maxhealth")).GetValueOrDefault(1f);
        return Math.Max(0f, (0.23f - valueOrDefault) * 1f / 0.18f);
    }
