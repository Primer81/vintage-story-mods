...
public interface IClientEventAPI : IEventAPI
{
    ...
    //
    // Summary:
    //     Called when a player joins. The Entity of the player might be null if out of
    //     range!
    ...
    //     The Entity of the player might be null if out of range!
    ...
    //
    // Summary:
    //     Fired when a player is ready to join but awaits any potential mod-user interaction,
    //     such as a character selection screen
    ...
    //     Allows for the event to be cancelled depending on the return value. Note: Not
    //     called when the server forcefully changes active slot.
    ...
    //
    // Summary:
    //     Fired when the GuiColorsPreset client setting is changed, since meshes may need
    //     to be redrawn
    event Action ColorsPresetChanged;

    //
    // Summary:
    //     Fired when server assets were received and all texture atlases have been created,
    //     also all sounds loaded
    event Action BlockTexturesLoaded;

    //
    // Summary:
    //     Fired when the player tries to reload the shaders (happens when graphics settings
    //     are changed)
    ...
    //
    // Summary:
    //     Provides low level access to the mouse down event. If e.Handled is set to true,
    //     the event will not be handled by the game
    event MouseEventDelegate MouseDown;

    //
    // Summary:
    //     Provides low level access to the mouse up event. If e.Handled is set to true,
    //     the event will not be handled by the game
    event MouseEventDelegate MouseUp;

    //
    // Summary:
    //     Provides low level access to the mouse move event. If e.Handled is set to true,
    //     the event will not be handled by the game
    event MouseEventDelegate MouseMove;

    //
    // Summary:
    //     Provides low level access to the key down event. If e.Handled is set to true,
    //     the event will not be handled by the game
    event KeyEventDelegate KeyDown;

    //
    // Summary:
    //     Provides low level access to the key up event. If e.Handled is set to true, the
    //     event will not be handled by the game
    ...
    //
    // Summary:
    //     Registers a custom itemstack renderer for given collectible object. If none is
    //     registered, the default renderer is used. For render target gui, the gui shader
    //     and its uniforms are already fully prepared, you may only call RenderMesh() and
    //     ignore the modelMat, position and size values - stack sizes however, are not
    //     covered by this.
    //
    // Parameters:
    //   forObj:
    ...
    //
    // Summary:
    //     Set up an asynchronous particle spawner. The async particle simulation does most
    //     of the work in a seperate thread and thus runs a lot faster, with the down side
    //     of not being exaclty in sync with player interactions. This method of spawning
    //     particles is best suited for ambient particles, such as rain fall.
    //
    // Parameters:
    //   handler:
