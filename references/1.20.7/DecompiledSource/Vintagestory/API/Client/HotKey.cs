...
public class HotKey
{
    //
    // Summary:
    //     For global hotkeys that shall not be blocked by anything (e.g. F11 for fullscreen,
    //     F12 for screenshot)
    public bool IsGlobalHotkey;

    //
    // Summary:
    //     For hotkeys that only available during a game session. When the game session
    //     ends, the handler to this hotkey is removed (or we have a dead reference to runninggame)
    ...
    //
    // Summary:
    //     If true, the handler will be called twice, once on the key or button down event,
    //     and once on the up event
    ...
    //
    // Summary:
    //     If this hotkey is a mouse control (primary or secondary mouse click), and this
    //     hotkey does not specifically require modifiers, then let's ignore modifiers when
    //     matching, because Shift+click or Ctrl+click is common; Alt+click might be used
