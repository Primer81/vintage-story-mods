...
public class GenBlockLayersFlat : ModSystem
{
	...
	{
		if (api.WorldManager.SaveGame.WorldType != "superflat")
		{
			return;
		}
		api.WorldManager.SaveGame.EntitySpawning = false;
		FlatWorldGenConfig flatwgenConfig = api.Assets.Get("worldgen/layers.json").ToObject<FlatWorldGenConfig>();
		List<int> blockIds = new List<int>();
		for (int i = 0; i < flatwgenConfig.blockCodes.Length; i++)
		{
			int blockId2 = api.WorldManager.GetBlockId(flatwgenConfig.blockCodes[i]);
			if (blockId2 != 0)
			{
	...
	private void OnChunkColumnGeneration(IChunkColumnGenerateRequest request)
	{
		...
		{
			chunks[y].Data.ClearBlocksAndPrepare();
		}
